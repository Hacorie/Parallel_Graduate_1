...
{
    int i, rc, done, rand_work_unit_len, total_work_units_handled;
    ...
    {        
        ...
        {
            printf("st2: unrecognized cmd-line arg at %d :%s:\n",i,argv[i]);
            exit(-1);
        }
    }
    if (min_work_unit_len < sizeof(double))
    {
        printf("st2: min len must be at least size of double %d\n",sizeof(double));
        exit(-1);
    }
    ...
    if (rc != MPI_SUCCESS)
    {
        printf("st2: MPI_Init_thread failed with rc=%d\n",rc);
        exit(-1);
    }
    switch (provided)
    {
    ...
    if ( my_world_rank == 0 )  /* if master app, put work */
    {
        ...
        dbgprintf(1,"st2: all work submitted after %f secs\n",MPI_Wtime()-start_job_time);
    }
    rc = MPI_Barrier( MPI_COMM_WORLD );
    ...
    while ( !done )
    {
        ...
        req_types[1] = -1;
        req_types[2] = -1;
        // dbgprintf( 1, "st2: reserving work\n" );
        temptime = MPI_Wtime();
        rc = ADLB_Reserve(req_types,&work_type,&work_prio,work_handle,&work_len,&answer_rank);
        // dbgprintf( 1, "st2: after reserve rc %d len %d type %d\n", rc, work_len, work_type );
        if ( rc == ADLB_DONE_BY_EXHAUSTION )
        {
            dbgprintf( 1, "st2: done by exhaustion\n" );
            break;
        }
        else if ( rc == ADLB_NO_MORE_WORK )
        {
            dbgprintf( 1, "st2: done by no more work\n" );
            break;
        }
        else if (rc < 0)
        {
            dbgprintf( 1, "st2: ** reserve failed, rc = %d\n", rc );
            ADLB_Abort(-1);
        }
        else if (work_type == WORK) 
        {
            ...
            {
                dbgprintf( 1, "st2: no more work on get_reserved\n" );
                break;
            }
            else   /* got good work */
            {
                ...
                dbgprintf(1,"st2: fakeworktime %f\n",time_for_fake_work);
                if (time_for_fake_work == 0.0)
                {
                    // dbgprintf(1,"st2: worktime 0.0\n");
                }
                else
                {
                    temptime = MPI_Wtime();
        ...
        {
            dbgprintf( 1, "st2: ** unexpected work type %d\n", work_type );
            ADLB_Abort( -1 );
        }
    }
    rc = MPI_Barrier( MPI_COMM_WORLD );
    ...
    // dbgprintf(1,"st2: total loop time %f\n",total_loop_time);
    total_work_time = end_work_time - start_work_time;
    dbgprintf(1,"st2: num handled by me %d\n",num_handled_by_me);
    dbgprintf(1,"st2: last end_work_time %f\n",end_work_time);
    dbgprintf(1,"st2: total work_time %f ; avg work time %f\n",
            total_work_time,total_work_time/((float)num_handled_by_me));
    dbgprintf(1,"st2: total reserve time %f ; avg reserve time %f\n",
            total_reserve_time,total_reserve_time/((float)num_handled_by_me));
    dbgprintf(1,"st2: total get time %f ; avg get time %f\n",
    ...
    if (my_world_rank == 0)
    {
        for (i=0; i < nranks; i++)
        {
            ...
            {
                dbgprintf(1,"st2: num handled by rank %d : total %d  avg work time %f\n",
                        i,num_handled_by_rank[i],
                        (total_work_time/(float)num_handled_by_rank[i]));
                total_work_units_handled += num_handled_by_rank[i];
            }
            else
                dbgprintf(1,"st2: num handled by rank %d : total 0  avg work time 0.0\n",i);
        }
        if (total_work_units_handled != num_work_units)
        {
            dbgprintf( 1, "st2: ** wrong number of work units handled: %d of %d\n",
    ...
    ADLB_Finalize();
    // printf("st2: calling mpi_finalize\n");
    MPI_Finalize();
    // printf("st2: past mpi_finalize\n");
